...
- 选歌主星级与结算星级在非 MS 时显示 OJN 等级除以 10，MS 时显示已保存的 mania 星级；结算使用成绩记录中的 Mod，旁边的 ruleset icon 颜色与悬停星级使用同一显示值。原生 `StarRating` 存储 mania 星级以供搜索与排序，O2Jam 星级独立存储。MS 在 mania 计分实现前仍保持隐藏占位状态。
